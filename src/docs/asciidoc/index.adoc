...
INFO:: NCBI uses a published private key (aspera_tokenauth_id_rsa) and a published passphrase (743128bf-3bf3-45b5-ab14-4602c67f2950) to allow people anonymous downloads

Also the plugin provides with a `ncbi_ascp` operator as a wrapper for the `ascp` function using `ncbi` as client
...
== Feature Requests and plugin support for custom Aspera clients.

This feature will be available in the Enterprise plugin version. If your Aspera server is an OpenData
instance, please contact us at jorge@incsteps.com to include it in the community version
